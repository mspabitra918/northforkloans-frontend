...
function DashboardDetailInner() {
  ...
  // The application id is now a real route segment (/dashboard/[id]) rather than
  // a ?id= query param. That makes "All applications" (→ /dashboard) and opening
  // an application (→ /dashboard/[id]) genuine route changes, which navigate
  // reliably — a query-only change on the same page did not always re-render
  // after the bankModalOpen deep-link was stripped with router.replace.
  ...
  // No application to show for this id.
  ...
  // URL with router.replace so the address bar is cleaned. Because `id` is now a
  // path segment (not a query param), this replace only rewrites the query
  // string and never touches later navigations to /dashboard.
  ...
  const load = useCallback(async () => {
    ...
    // This is the client-side half of the fix; the backend independently
